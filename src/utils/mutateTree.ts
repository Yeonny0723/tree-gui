...
import { confirmFn } from './checkCommand';
...
export const recursiveAddFile: I.IRecursiveaddFile = (fileTree, fileType, filePath) => {
   ...
   for (let i = 1; i < filePathLst.length + 1; i++) {
      ...
      if (target === undefined) {
         // 내부 {}에 이미 다른 노드 존재하여 target === undefined인 경우
         const newNode = {
...
export const move: I.Imove = (fileTree, fromPath, toPath) => {
   ...
   filePathLst[0] = '/';
   _.set(fileTree, [...filePathLst, 'next'], { [fromPath.split('/').slice(-1)[0]]: target }); // 이동할 브랜치로 삽입
};
